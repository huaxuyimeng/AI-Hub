...
//
// 职责：用 LangGraph StateGraph 表达"发言 → 决策继续/结束 → 主持人汇总"状态机。
//
// 流程：
//   __start__ → nextSpeaker → callLLM → decideContinue
//                                            │
//                            ┌───────────────┴───────────────┐
//                            ▼                               ▼
...
//                            │                               │
//                            ▼                               ▼
//                       nextSpeaker                      conclude → END
//                       (loop back)
//
// 关键决策（见文档 01）：
//   - D10：不依赖 LangGraph checkpoint，每次 node 完成后由 tRPC 层写 DB
//   - D11：MAX_MEETING_ROUNDS = 3 硬上限
//   - D9：transcripts 永远追加（即使失败也写 error entry）
//
// 过期条件：
//   - LangGraph 升级到 1.x → 检查 StateGraph / Annotation API 兼容性
//   - chatLC 签名变化 → Step 3 同步调整
//   - decideContinue 决策策略变更 → makeDecision 函数
 ...
 * - errors：拼接数组
 */
const MeetingAnnotation = Annotation.Root({
...
function accumulateUsage(
  prev: MeetingState['usageTotal'] | undefined,
  modelName: string,
  inputTokens: number,
  outputTokens: number,
 ...
 */
function nextSpeakerNode(state: MeetingState): Partial<MeetingState> {
 ...
 */
async function decideContinueNode(
  state: MeetingState,
  config: LangGraphRunnableConfig
): Promise<Partial<MeetingState>> {
 ...
 * @param ctx     运行时上下文（tenantId + hostModel + 可选 decisionModel）
 * @returns       最终状态（包含 conclusion + errors）
 */
